...
package mode (from src/) and stitched mode (from dist/apathetic_testing.py).

The test verifies that:
- Imports from apathetic_testing work correctly
...
def test_import_semantics_work_in_all_runtime_modes() -> None:
    ...
    the module is loaded from src/ (package) or dist/apathetic_testing.py
    (stitched).
    """
    # --- execute ---
