...
Thanks for your interest in contributing to **Apathetic Python Testing** — a collection of utility functions for Apathetic Tools projects.  
This guide explains how to set up your environment, run checks, and safely contribute code.

## Supported Python Versions

Apathetic Python Testing targets **Python 3.10+**.  
...
Navigate to `http://localhost:4000/python-testing/`

> [!NOTE]
> The baseurl is `/python-testing`, so make sure to include it in the local URL.
...
Apathetic Python Testing ships in three forms:

| Target | Command | Output |
|--------|---------|--------|
| **Stitched script** | `poetry run poe build:stitched` | Creates `dist/apathetic_testing.py` (human-readable, stitched source) |
| **Zipapp** | `poetry run poe build:zipapp` | Creates `dist/apathetic_testing.pyz` (dependency-aware, importable) |
...
2. **If a release is needed:**
   ...
   - Uploads custom assets (`apathetic_testing.py` and `apathetic_testing.pyz`)
...
- `dist/apathetic_testing.py` — single-file script (human-readable, stitched source)
- `dist/apathetic_testing.pyz` — zipapp format (dependency-aware, importable)
...
**Thank you for helping keep Apathetic Python Testing minimal, dependency-free, and delightful.**
